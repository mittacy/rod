...
var (
	...
	errPageConnUnNormal = errors.New("the target can't access to the originating window")
)
...
/**
 * 1. 等待空闲连接
 * 2. 检查空闲连接，超出空闲时间的连接执行释放
 * 3. 从空闲连接头部取一个，然后检查连接状态、连接创建时间是否超出限制，通过则直接返回连接（期间如果有不符合的连接，则执行关闭）
 * 4. 如果空闲链表没有可用连接，则创建连接再返回
 */
func (p *Pool) GetWithCtx(ctx context.Context) (*PoolConn, error) {
	...
	if p.closed {
		p.mu.Unlock()
		err := errors.New("rod-pool: get on closed pool")
		return nil, err
	}
...
func (p *Pool) TestOnBorrow(pc *PoolConn) error {
	info, err := pc.page.Info()
	if err != nil {
		return err
	}
	if !info.CanAccessOpener {
		return errPageConnUnNormal
	}
	return nil
}
...
func (p *Pool) newConn() (*PoolConn, error) {
	page, err := p.newPage()
	if err != nil {
		return nil, err
	}
